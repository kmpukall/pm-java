...
public class PMSelection {
	...
	public boolean isSaneSelection() {
		return _selectedPropertyDescriptor != null
				|| _singleSelectedNode != null;
	}

	private void findSelection(int offset, int length) {
		PMExactSelectionVisitor selectionVisitor = new PMExactSelectionVisitor(
				offset, length);
		_compilationUnit.accept(selectionVisitor);

		ASTNode containingNode = selectionVisitor.getContainingNode();

		if (containingNode.getStartPosition() == offset
				&& containingNode.getLength() == length)
			...
			PMInsertionPoint startInsertionPoint = new PMInsertionPoint(
					_compilationUnit, offset);

			PMInsertionPoint endInsertionPoint = new PMInsertionPoint(
					_compilationUnit, offset + length);

			if (startInsertionPoint.isSaneInsertionPoint()
					&& endInsertionPoint.isSaneInsertionPoint()) {

				if (startInsertionPoint.insertionParent() == endInsertionPoint
						.insertionParent()
					...
					_selectedPropertyDescriptor = startInsertionPoint
							.insertionProperty();

					_childListPropertyOffset = startInsertionPoint
							.insertionIndex();

					_childListPropertyLength = endInsertionPoint
							.insertionIndex()
	...
	protected static class PMExactSelectionVisitor extends
		...
		protected boolean nodeContainsSelection(ASTNode node, int offset,
				int length) {
			return offset >= node.getStartPosition()
					&& offset + length <= node.getStartPosition()
							+ node.getLength();
		}
	...
	public String getSelectionAsString() {
		...
		try {
			...
			if (iCompilationUnit != null)
				entireSource = ((ICompilationUnit) _compilationUnit
			...
			System.err
					.println("Exception in PMSelection.getSelectionAsString(): "
							+ e);
