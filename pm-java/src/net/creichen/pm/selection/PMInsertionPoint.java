...
public class PMInsertionPoint {
	...
	private boolean findInsertionPointUnderNode(ASTNode parentNode,
		...
		if (statementCount > 0) {
			if (offset <= statements.get(0).getStartPosition())
				...
				if (offset >= lastStatement.getStartPosition()
						+ lastStatement.getLength()) {
					...
					for (int i = 1; i < statementCount; i++) {
						...
						if (offset >= statement1.getStartPosition()
								+ statement1.getLength()
	...
	private boolean findInsertionPointInBlock(int offset) {
		...
		Block containingBlock = FindContainingBlockForSelection(
				_compilationUnit, offset, 0);

		if (containingBlock != null) {
			return findInsertionPointUnderNode(containingBlock,
					Block.STATEMENTS_PROPERTY, offset);
	...
	static protected class PMContainingNodeVisitor extends ASTVisitor {
		...
		public boolean visitContainingNode(ASTNode node) {
			if (node.getStartPosition() + 1 <= _offset
					&& _offset + _length <= node.getStartPosition()
							+ node.getLength() - 1) {
	...
	private static Block FindContainingBlockForSelection(ASTNode nodeToSearch,
			int offset, int length) {

		PMContainingBlockVisitor containingBlockVisitor = new PMContainingBlockVisitor(
				offset, length);
	...
	static protected class PMContainingBlockVisitor extends
			PMContainingNodeVisitor {
	...
	private static TypeDeclaration FindContainingTypeDeclarationForSelection(
			ASTNode nodeToSearch, int offset, int length) {
		...
		return (TypeDeclaration) containingTypeDeclarationVisitor
				.getContainingNode();
	}

	static protected class PMContainingTypeDeclarationVisitor extends
			PMContainingNodeVisitor {
