...
public class PMASTMatcherTest {
	...
	@Test public void testMatchIsomorphicSimpleNames() {
	...
	@Test public void testMatchNonIsomorphicSimpleNames() {
		AST ast = AST.newAST(AST.JLS3);
	...
	@Test public void testMatchIsomorphicCompilationUnits() {
		...
		assertSame(isomorphicNodes.get(secondY1.getParent().getParent().getParent()), secondY2.getParent().getParent().getParent());
	
		assertNotNull(isomorphicNodes.get(secondY1.getParent().getParent().getParent().getParent()));
		assertSame(isomorphicNodes.get(secondY1.getParent().getParent().getParent().getParent()), secondY2.getParent().getParent().getParent().getParent());

	}
	
	@Test public void testMatchNonIsomorphicInSimplePropertyCompilationUnits() {
		ASTParser parser = ASTParser.newParser(AST.JLS3);
		parser.setSource("public class S { void m() {int x; x++; int y; y++;} }".toCharArray());
	...
	@Test public void testMatchNonIsomorphicInChildListPropertyCompilationUnits() {
		ASTParser parser = ASTParser.newParser(AST.JLS3);
		parser.setSource("public class S { void m() {int x; x++; int y; y++;} }".toCharArray());
