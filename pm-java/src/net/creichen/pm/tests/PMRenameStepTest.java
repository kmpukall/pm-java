...
public class PMRenameStepTest extends PMTest {
	...
	@Test public void testLocalWithMethodInvocation() throws JavaModelException {
		...
		SimpleName firstSInT = PMASTQuery.simpleNameWithIdentifierInMethodInClassInCompilationUnit("s", 0, "m", 0, "T", 0, (CompilationUnit)pmProject.findASTRootForICompilationUnit(compilationUnitT))	;
		...
		assertTrue(compilationUnitSourceMatchesSource("public class T {void m() {S sInstance = new S(); sInstance.sMethod();} }", compilationUnitT.getSource()));		

		IProblem[] problemsT = ((CompilationUnit)pmProject.findASTRootForICompilationUnit(compilationUnitT)).getProblems();
		
		assertEquals(0, problemsT.length);
	}
	
	
	@Test public void testRenameClassToNewName() throws JavaModelException {
		String sourceS = "public class S {void sMethod() {}}";
		...
		TypeDeclaration classS = PMASTQuery.classWithNameInCompilationUnit("S", 0, (CompilationUnit)pmProject.findASTRootForICompilationUnit(compilationUnitS));
		...
		ICompilationUnit compilationUnitT = pmCompilationUnitS.getICompilationUnit();
		
		assertTrue(compilationUnitSourceMatchesSource("public class T {void sMethod() {}}", compilationUnitT.getSource()));		

		IProblem[] problems = ((CompilationUnit)pmProject.findASTRootForICompilationUnit(compilationUnitT)).getProblems();
	
		assertEquals(0, problems.length);
	}
	
	@Test public void testRenameLocalToShadowField() throws JavaModelException {
		String sourceS = "public class S {" +
				"String iVar;" +
				"void m() {" +
				"String lVar;" +
				"iVar.length();" +
		...
		SimpleName name = PMASTQuery.simpleNameWithIdentifierInMethodInClassInCompilationUnit("lVar", 0, "m", 0, "S", 0, pmCompilationUnitS.getASTNode());
		...
		String expectedNewSourceS = "public class S {" +
								"String iVar;" +
								"void m() {" +
									"String iVar;" +
									"iVar.length();" +
	...
	@Test public void testRenameClassWithConstructor() {
		...
		TypeDeclaration classNode = PMASTQuery.classWithNameInCompilationUnit("A", 0, pmCompilationUnitS.getASTNode());
	...
	@Test public void testRenameConstructor() {
		//We expect the class and all other constructors and calls of that construtor will also be renamed
		
		String source = "public class A {protected int x; public A(int y) {x=y;} public A() {x = 0; A a = new A(); a = new A(x);} public static class InnerA extends A { } }";
		
		ICompilationUnit compilationUnitS =  createNewCompilationUnit("", "A.java", source);
		...
		MethodDeclaration methodDeclaration = PMASTQuery.methodWithNameInClassInCompilationUnit("A", 0, "A", 0, pmCompilationUnitS.getASTNode());
	...
	@Test public void testRenameConstructorFollowedByRenameClass() {
		...
		MethodDeclaration methodDeclaration = PMASTQuery.methodWithNameInClassInCompilationUnit("S", 0, "S", 0, pmCompilationUnitS.getASTNode());
		
		SimpleName methodName = methodDeclaration.getName();
		
		PMRenameStep renameConstructorStep = new PMRenameStep(pmProject, methodName);
		...
		assertTrue(compilationUnitSourceMatchesSource(expectedNewSourceAfterRenameConstructor, pmCompilationUnitS.getSource()));		

		TypeDeclaration classNode = PMASTQuery.classWithNameInCompilationUnit("T", 0, pmCompilationUnitS.getASTNode());
	...
	@Test public void testRenameClassFollowedByRenameConstructor() {
		...
		TypeDeclaration classNode = PMASTQuery.classWithNameInCompilationUnit("S", 0, pmCompilationUnitS.getASTNode());
		...
		MethodDeclaration methodDeclaration = PMASTQuery.methodWithNameInClassInCompilationUnit("T", 0, "T", 0, pmCompilationUnitS.getASTNode());
	...
	@Test public void testRenameConstructorInComplicatedClass() {
		String sourceA = 
		"package testpackage;"
		+ "public class A {"
		+ 		"protected int x;"
		...
		ICompilationUnit compilationUnitA =  createNewCompilationUnit("testpackage", "A.java", sourceA);
		ICompilationUnit compilationUnitB = createNewCompilationUnit("testpackage", "B.java", sourceB);
		
		PMProject pmProject = PMWorkspace.sharedWorkspace().projectForIJavaProject(_iJavaProject);
		
		PMCompilationUnit pmCompilationUnitA = pmProject.getPMCompilationUnitForICompilationUnit(compilationUnitA);
		PMCompilationUnit pmCompilationUnitB = pmProject.getPMCompilationUnitForICompilationUnit(compilationUnitB);

		
		MethodDeclaration methodDeclaration = PMASTQuery.methodWithNameInClassInCompilationUnit("A", 0, "A", 0, pmCompilationUnitA.getASTNode());
		...
		String expectedTransformedSourceA = 
			"package testpackage;"
			+ "public class x__5 {"
			+ 		"protected int x;"
			+		"protected int unused;"		
		...
		String expectedTransformedSourceB =
			"package testpackage;"
	...
	@Test public void testRenameConstructorInClassInPackageWithSubclass() {
		...
		ICompilationUnit compilationUnitA =  createNewCompilationUnit("testpackage", "A.java", sourceA);
		 ...
		 ICompilationUnit compilationUnitB = createNewCompilationUnit("", "B.java", sourceB);
	...
	@Test public void testRenameClassViaExtendsClause() {
		String sourceA = "public class A {public A() {} public static class InnerA extends A {} }";
		
		ICompilationUnit compilationUnitA =  createNewCompilationUnit("", "A.java", sourceA);
		
		PMProject pmProject = PMWorkspace.sharedWorkspace().projectForIJavaProject(_iJavaProject);
		
		PMCompilationUnit pmCompilationUnitA = pmProject.getPMCompilationUnitForICompilationUnit(compilationUnitA);

		TypeDeclaration outerClassDeclaration = PMASTQuery.classWithNameInCompilationUnit("A", 0, pmCompilationUnitA.getASTNode());

		TypeDeclaration innerAClassDeclaration = (TypeDeclaration)outerClassDeclaration.bodyDeclarations().get(1);
	
		SimpleName extendsClassName = (SimpleName)((SimpleType)innerAClassDeclaration.getSuperclassType()).getName();
	...
	//Should also test via static class name like Foo.doSomething() and via this expression (Foo.this) and qualified name (Foo.something) in inner class to get to outer class
}
