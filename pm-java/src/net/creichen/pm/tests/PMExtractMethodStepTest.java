...
public class PMExtractMethodStepTest extends PMTest {
	...
	@Test public void testGetNamesToExtract() {
		...
		assertEquals(PMASTQuery.simpleNameWithIdentifierInMethodInClassInCompilationUnit("i", 0, "m", 0, "S", 0, pmCompilationUnitS.getASTNode()),
																						namesToExtract.get(0));
		
		assertEquals(PMASTQuery.simpleNameWithIdentifierInMethodInClassInCompilationUnit("j", 1, "m", 0, "S", 0, pmCompilationUnitS.getASTNode()),
				namesToExtract.get(1));

	}
	
	@Test public void testExtractLocalVariableExpression() {
		String source = "class S {String _s; void m(int i) {int j; System.out.println(_s + i + j);}}";
