...
public class PMExtractMethodStep extends PMStep {
	...
	public PMExtractMethodStep(PMProject project, Expression expression) {
		...
		//System.err.println("_extractedMethodDeclaration is " + _extractedMethodDeclaration);
		
		//System.err.println("_replacementMethodInvocation is " + _replacementMethodInvocation);
	}
	...
	protected MethodDeclaration newMethodDeclaration() {
		...
		newMethodDeclaration.setReturnType2(PMExtractMethodStep.newTypeASTNodeForTypeBinding(ast, _originalExpression.resolveTypeBinding()));
	...
	protected MethodInvocation newMethodInvocation() {
		...
		newMethodInvocation.setName(ast.newSimpleName(_extractedMethodDeclaration.getName().getIdentifier()));
	...
	public Map<ICompilationUnit, ASTRewrite> calculateTextualChange() {
		...
		ListRewrite lrw = astRewrite.getListRewrite(containingClass, TypeDeclaration.BODY_DECLARATIONS_PROPERTY);
		...
		result.put(_project.findPMCompilationUnitForNode(_originalExpression).getICompilationUnit(), astRewrite);
