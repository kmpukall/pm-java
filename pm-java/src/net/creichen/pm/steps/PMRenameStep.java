...
public class PMRenameStep extends PMStep {
	...
	public PMRenameStep(PMProject project, SimpleName nameNode) {
		...
		if (nameNode != null) {
			...
			throw new RuntimeException(
					"Cannot create PMRenameStep with null nameNode");
	...
	public Map<ICompilationUnit, ASTRewrite> calculateTextualChange() {
		...
		ArrayList<SimpleName> nodesToRename = nameModel
				.nameNodesRelatedToNameNode(_nameNode);
		...
		for (SimpleName nodeToRename : nodesToRename) {
			CompilationUnit containingCompilationUnit = (CompilationUnit) nodeToRename
					.getRoot();

			ICompilationUnit containingICompilationUnit = (ICompilationUnit) containingCompilationUnit
					.getJavaElement();

			List<SimpleName> namesForUnit = nodesByICompilationUnit
					.get(containingICompilationUnit);

			if (namesForUnit == null) {
				namesForUnit = new ArrayList<SimpleName>();
				nodesByICompilationUnit.put(containingICompilationUnit,
						namesForUnit);
			}
		...
		if (nodesByICompilationUnit.size() > 0) {
			for (ICompilationUnit unitForRename : nodesByICompilationUnit
					.keySet()) {

				List<SimpleName> nodesInUnit = nodesByICompilationUnit
						.get(unitForRename);

				ASTRewrite astRewrite = ASTRewrite.create(nodesInUnit.get(0)
						.getAST());
				...
				for (SimpleName sameNode : nodesInUnit) {
					...
					SimpleName newNode = _nameNode.getAST().newSimpleName(
							_newName);
