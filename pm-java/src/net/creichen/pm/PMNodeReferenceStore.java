...
public class PMNodeReferenceStore {
	...
	public PMNodeReference getReferenceForNode(ASTNode node) {
		...
		WeakReference<PMNodeReference> weakReference = _referencesForNodes
				.get(node);
	...
	public void replaceOldNodeVersionWithNewVersion(ASTNode oldNode,
			ASTNode newNode) {
		WeakReference<PMNodeReference> referenceWeakRef = _referencesForNodes
				.get(oldNode);
	...
	private static String generatedIdentifierForNode(ASTNode node) {
		...
		return node.getClass().getName().toString() + ":"
	...
	public class PMUUIDNodeReference implements PMNodeReference {
		...
		private PMUUIDNodeReference(ASTNode node) {
			...
			_description = PMNodeReferenceStore
					.generatedIdentifierForNode(node);
