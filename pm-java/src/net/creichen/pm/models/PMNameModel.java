...
public class PMNameModel {
			...
			rootNode.accept(new ASTVisitor() {
				...
				public boolean visit(SimpleName simpleName) {
					...
					if (binding instanceof IVariableBinding) {
						...
						binding = ((IVariableBinding) binding)
						 ...
						 * 
						 * If we don't use the getVariableDeclaration, we get
						 * separate bindings for the first and second use of
						 * _ivar, which is clearly not what we want.
						 * 
						 * But, this may lose information that we want in other
						 * cases
						 */
	...
	protected ArrayList<SimpleName> nameNodesRelatedToNameNodeWithIdentifier(
			String identifier) {
		...
		for (ASTNode rootNode : _project.getASTRoots()) {
			rootNode.accept(new ASTVisitor() {
				public boolean visit(SimpleName visitedNode) {
					String identifierForVisitedNode = _identifiersForNames
							.get(visitedNode);
	...
	protected List<MethodDeclaration> constructorsForClass(
	...
	protected Set<SimpleName> representativeNameNodesIndirectlyRelatedToNameNode(
		...
		StructuralPropertyDescriptor locationInParent = nameNode
				.getLocationInParent();

		if (parent instanceof TypeDeclaration
				&& locationInParent == ((TypeDeclaration) parent)
						.getNameProperty()) {
			...
			if (method.isConstructor()) {
				...
				TypeDeclaration containingClass = (TypeDeclaration) method
						.getParent();
		...
		for (SimpleName directlyRelatedName : directlyRelatedNodes) {
			if (!visitedNodes.contains(directlyRelatedName)) {
				...
				for (SimpleName indirectlyRelatedName : indirectlyRelatedNames) {
					recursiveAddNameNodesRelatedToNameNode(
							indirectlyRelatedName, visitedNodes);
				}
			}
		}
	...
	public Set<PMInconsistency> calculateInconsistencies() {
		...
		for (PMCompilationUnit pmCompilationUnit : _project
				.getPMCompilationUnits()) {

			inconsistencies
					.addAll(nameInconsistenciesForICompilationUnit(pmCompilationUnit));
		}
	...
	private Set<PMInconsistency> nameInconsistenciesForICompilationUnit(
		...
		for (SimpleName simpleName : simpleNamesInCompilationUnit) {
			...
			ASTNode declaringNode = _project
					.findDeclaringNodeForName(simpleName);// declaringModel.getCompilationUnit().findDeclaringNode(simpleName.resolveBinding());

			if (declaringNode != null) {
				SimpleName declaringSimpleName = _project
				...
				if (usingIdentifier == null) {
					inconsistencies.add(new PMUnknownName(_project,
							pmCompilationUnit, simpleName));
				} else {
					if (declaringIdentifier != usingIdentifier
						...
						inconsistencies.add(new PMNameCapture(_project,
								pmCompilationUnit, simpleName, null,
								declaringNode));
					}
				}

				if (!declaringSimpleName.getIdentifier().equals(
						simpleName.getIdentifier())) {
					inconsistencies.add(new PMNameConflict(_project,
							pmCompilationUnit, simpleName, declaringSimpleName
									.getIdentifier()));
				}
	...
	private Set<SimpleName> simpleNamesInCompilationUnit(
