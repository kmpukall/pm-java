...
public class PMRenameProcessorTest extends PMTest {
	...
	public void testCreateNewCompilationUnitInTest() {
		...
		assertTrue(pmProject
				.getPMCompilationUnitForICompilationUnit(iCompilationUnit) != null);

	}

	@Test
	public void testRenameLocalVariableViaDeclaration()
		...
		PMRenameProcessor renameFooToStan = new PMRenameProcessor(
				new TextSelection(37, 3), iCompilationUnit);
		...
		PMRenameProcessor renameStanToBar = new PMRenameProcessor(
				new TextSelection(37, 4), iCompilationUnit);
	...
	public void testRenameLocalVariableViaUse() throws JavaModelException {
		...
		PMRenameProcessor renameFooToStan = new PMRenameProcessor(
				new TextSelection(42, 3), iCompilationUnit);
		...
		PMRenameProcessor renameStanToBar = new PMRenameProcessor(
				new TextSelection(43, 4), iCompilationUnit);
	...
	public void testRenameIVarCapturesLocal() throws JavaModelException {
		...
		PMRenameProcessor renameIvarToBar = new PMRenameProcessor(
				new TextSelection(22, 3), iCompilationUnit);
		...
		assertEquals(
				"Foo.java",
				"public class Foo {int bar; void method() {int bar; bar = 5;} }",
				iCompilationUnit.getSource());
		...
		ASTNode expectedCapturedNode = PMASTQuery
				.simpleNameWithIdentifierInMethodInClassInCompilationUnit(
						"bar", 1, "method", 0, "Foo", 0, parsedCompilationUnit);
		...
		ASTNode expectedCapturingNode = PMASTQuery
				.simpleNameWithIdentifierInMethodInClassInCompilationUnit(
						"bar", 0, "method", 0, "Foo", 0, parsedCompilationUnit)
				.getParent();
	...
	public void testRenameThroughCapture() throws JavaModelException {
		...
		PMRenameProcessor renameFooToBar = new PMRenameProcessor(
				new TextSelection(37, 3), iCompilationUnit);
		...
		assertEquals(
				"public class Foo {void method() {int bar; int bar; bar = 5; bar = 6;} }",
				iCompilationUnit.getSource());
		...
		ASTNode expectedCapturedNode = pmProject.nodeForSelection(
				new TextSelection(51, 3), iCompilationUnit);
		assertEquals(expectedCapturedNode, nameCapture.getNode());
		...
		PMRenameProcessor renameBarToFoo = new PMRenameProcessor(
				new TextSelection(60, 3), iCompilationUnit);
		...
		assertEquals(
				"public class Foo {void method() {int bar; int foo; bar = 5; foo = 6;} }",
				iCompilationUnit.getSource());
	...
	public void testRenameInMultipleFiles() throws JavaModelException {
		...
		PMRenameProcessor renameXToY = new PMRenameProcessor(new TextSelection(
				32, 1), unit1);
		...
		assertEquals(
				"public class Unit1 { public int y; void method() {y++;} }",
				unit1.getSource());
		assertEquals(
		...
		for (PMInconsistency inconsistency : pmProject.allInconsistencies()) {
			...
			if (inconsistency instanceof PMMissingDefinition)
				System.out.println("For definition of class: "
						+ ((PMMissingDefinition) inconsistency)
								.getDefiningNode().getClass());
		}
	...
	public void testAcceptNameCapture() throws JavaModelException {
		...
		Set<PMInconsistency> newInconsistencies = pmProject
				.allInconsistencies();
