...
public class PMASTQueryTest extends PMTest {
	...
	@Test public void testFindClassByName() {
	...
	@Test public void testFindMethodByName() {
		...
		assertEquals(secondF, PMASTQuery.methodWithNameInClassInCompilationUnit("f", 1, "S", 1, compilationUnit));
	}
	
	@Test public void testFindFieldByName() {
		
		String source = "class S {int x,y; int f() {} int y; int x,y,x; int y; int y,x; int x; }";
		
		CompilationUnit compilationUnit = parseCompilationUnitFromSource(source);
		
		ASTNode field = PMASTQuery.nodeForSelectionInCompilationUnit(60, 1, compilationUnit).getParent(); // the finder finds the SimpleName, so we need to get the parent fragment
		
		assertTrue(field instanceof VariableDeclarationFragment);	
		
		assertEquals(field, PMASTQuery.fieldWithNameInClassInCompilationUnit("x", 3, "S", 0, compilationUnit));
	}
	
@Test public void testFindLocalByName() {
		
		String source = "class S {int x,y; int f() {int x,y; try {} catch(Exception x){} while(1) {int y,x;} } }";
		
		CompilationUnit compilationUnit = parseCompilationUnitFromSource(source);
		
		ASTNode local = PMASTQuery.nodeForSelectionInCompilationUnit(80, 1, compilationUnit).getParent(); // the finder finds the SimpleName, so we need to get the parent fragment
		
		assertTrue(local instanceof VariableDeclarationFragment);	
		
		assertEquals(local, PMASTQuery.localWithNameInMethodInClassInCompilationUnit("x", 2, "f", 0, "S", 0, compilationUnit));
	}
	

@Test public void testFindSimpleNameByIdentifier() {
	...
	assertEquals(simpleName, PMASTQuery.simpleNameWithIdentifierInMethodInClassInCompilationUnit("x", 4, "f", 0, "S", 0, compilationUnit));
}
