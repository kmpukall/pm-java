...
public class PMCopyStepTest extends PMTest {
	...
	@Test
	public void testCopyFieldCreatesNewNameIdentifier()
		...
		FieldDeclaration fieldDeclaration = (FieldDeclaration) fieldDeclarationFragment
				.getParent();

		SimpleName fieldDeclarationOriginalName = ((VariableDeclarationFragment) fieldDeclaration
				.fragments().get(0)).getName();

		String fieldDeclarationOriginalNameIdentifier = pmProject
				.getNameModel().identifierForName(fieldDeclarationOriginalName);
		...
		assertTrue(compilationUnitSourceMatchesSource(
				"public class S {int x; void m(){x = 1;}}",
				iCompilationUnit.getSource()));

		FieldDeclaration fieldDeclarationCopy = (FieldDeclaration) pmProject
				.getPasteboard().getPasteboardRoots().get(0);

		assertNotNull(fieldDeclarationCopy);

		SimpleName fieldDeclarationCopyName = ((VariableDeclarationFragment) fieldDeclarationCopy
				.fragments().get(0)).getName();

		String fieldDeclarationCopyNameIdentifier = pmProject.getNameModel()
	...
	public void testCopyDeclarationAndReferencesMaintainsInternalReferences()
		...
		CompilationUnit compilationUnitS = (CompilationUnit) pmProject
				.findASTRootForICompilationUnit(iCompilationUnit);

		FieldDeclaration fieldDeclaration = (FieldDeclaration) PMASTQuery
				.fieldWithNameInClassInCompilationUnit("x", 0, "S", 0,
						compilationUnitS).getParent();
		SimpleName fieldDeclarationOriginalName = ((VariableDeclarationFragment) fieldDeclaration
				.fragments().get(0)).getName();
		String fieldDeclarationOriginalNameIdentifier = nameModel
		...
		String methodDeclarationOriginalNameIdentifier = nameModel
				.identifierForName(methodDeclarationOriginalName);

		SimpleName originalUseOfYInM = PMASTQuery
				.simpleNameWithIdentifierInNode("y", 0,
						methodDeclaration.getBody());
		String originalUseOfYInMIdentifier = nameModel
				.identifierForName(originalUseOfYInM);
		...
		MethodDeclaration methodDeclarationCopy = (MethodDeclaration) pmProject
				.getPasteboard().getPasteboardRoots().get(0);
		FieldDeclaration fieldDeclarationCopy = (FieldDeclaration) pmProject
		...
		String fieldDeclarationCopyNameIdentifier = nameModel
				.identifierForName(fieldDeclarationCopyName);
		assertNotNull(fieldDeclarationCopyNameIdentifier);
		assertFalse(fieldDeclarationCopyNameIdentifier
				.equals(fieldDeclarationOriginalNameIdentifier));
		...
		String methodDeclarationCopyNameIdentifier = nameModel
		...
		SimpleName copyUseOfXInM = PMASTQuery.simpleNameWithIdentifierInNode(
				"x", 0, methodDeclarationCopy.getBody());
		String copyUseOfXInMIdentifier = nameModel
				.identifierForName(copyUseOfXInM);

		assertEquals(copyUseOfXInMIdentifier,
				fieldDeclarationCopyNameIdentifier);

		// use of external field declaration (i.e. one not copied) still points
		// to its original declaration

		SimpleName copyUseOfYInM = PMASTQuery.simpleNameWithIdentifierInNode(
				"y", 0, methodDeclarationCopy.getBody());
		String copyUseOfYInMIdentifier = nameModel
				.identifierForName(copyUseOfYInM);
	...
	public void testCopyDefinitionAndReferencesMaintainsInternalReferences()
		...
		ExpressionStatement fourthStatementOriginal = (ExpressionStatement) methodDeclaration
				.getBody().statements().get(3);
		Assignment xGetsXPlusOneAssignmentOriginal = (Assignment) fourthStatementOriginal
				.getExpression();

		SimpleName x3RHSOriginal = PMASTQuery.simpleNameWithIdentifierInNode(
				"x", 1, xGetsXPlusOneAssignmentOriginal);

		PMNodeReference x3RHSOriginalNodeReference = pmProject
				.getReferenceForNode(x3RHSOriginal);

		ExpressionStatement fifthStatementOriginal = (ExpressionStatement) methodDeclaration
		...
		assertTrue(compilationUnitSourceMatchesSource(source,
				iCompilationUnit.getSource()));

		ExpressionStatement fourthStatementCopy = (ExpressionStatement) pmProject
				.getPasteboard().getPasteboardRoots().get(1);
		assertNotNull(fourthStatementCopy);
		ExpressionStatement fifthStatementCopy = (ExpressionStatement) pmProject
				.getPasteboard().getPasteboardRoots().get(0);
		assertNotNull(fifthStatementCopy);

		Assignment xGetsXPlusOneAssignmentCopy = (Assignment) fourthStatementCopy
				.getExpression();

		PMNodeReference xGetsXPlusOneAssignmentCopyReference = pmProject
				.getReferenceForNode(xGetsXPlusOneAssignmentCopy);

		SimpleName x3RHSCopy = PMASTQuery.simpleNameWithIdentifierInNode("x",
				1, xGetsXPlusOneAssignmentCopy);
		PMNodeReference x3RHSCopyNodeReference = pmProject
				.getReferenceForNode(x3RHSCopy);

		Set<PMNodeReference> definitionsForX3RHSOriginal = udModel
				.definitionIdentifiersForName(x3RHSOriginalNodeReference);
		Set<PMNodeReference> definitionsForX3RHSCopy = udModel
		...
		SimpleName x4RHSCopy = PMASTQuery.simpleNameWithIdentifierInNode("x",
				0, fifthStatementCopy);
		PMNodeReference x4RHSCopyNodeReference = pmProject
				.getReferenceForNode(x4RHSCopy);

		Set<PMNodeReference> definitionsForX4RHSCopy = udModel
				.definitionIdentifiersForName(x4RHSCopyNodeReference);

		assertEquals(1, definitionsForX4RHSCopy.size());
		assertEquals(xGetsXPlusOneAssignmentCopyReference,
				definitionsForX4RHSCopy.toArray()[0]);
	}
