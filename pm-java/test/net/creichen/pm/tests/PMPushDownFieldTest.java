...
public class PMPushDownFieldTest extends PMTest {
	...
	public void testPushDownFieldWithNoUses() throws JavaModelException {
		ICompilationUnit iCompilationUnitS = createNewCompilationUnit("",
				"S.java", "public class S { int _y;}");
		ICompilationUnit iCompilationUnitT1 = createNewCompilationUnit("",
				"T1.java", "public class T1 extends S {  }");
		ICompilationUnit iCompilationUnitT2 = createNewCompilationUnit("",
				"T2.java", "public class T2 extends S {  }");
		...
		TypeDeclaration classT1 = PMASTQuery.classWithNameInCompilationUnit(
				"T1", 0, compilationUnitT1);

		PMPasteStep pasteStep1 = new PMPasteStep(project, classT1,
				classT1.getBodyDeclarationsProperty(), classT1
						.bodyDeclarations().size());
		classT1 = null;

		pasteStep1.applyAllAtOnce();

		assertEquals(new HashSet<PMInconsistency>(),
				project.allInconsistencies());

		yField = (FieldDeclaration) PMASTQuery
				.fieldWithNameInClassInCompilationUnit(
						"_y",
						0,
						"S",
		...
		TypeDeclaration classT2 = PMASTQuery.classWithNameInCompilationUnit(
				"T2", 0, compilationUnitT2);

		PMPasteStep pasteStep2 = new PMPasteStep(project, classT2,
				classT2.getBodyDeclarationsProperty(), classT2
						.bodyDeclarations().size());
		classT2 = null;
		...
		assertTrue(compilationUnitSourceMatchesSource(
				"public class T1 extends S { int _y;  }",
				iCompilationUnitT1.getSource()));
		assertTrue(compilationUnitSourceMatchesSource(
				"public class T2 extends S { int _y;  }",
